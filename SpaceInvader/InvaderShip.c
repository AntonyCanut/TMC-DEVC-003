...
void UpdateInvaderShip() {
    if (countUpdateInvaderShip % 17 == 0) {
    ...
    if (countUpdateInvaderShip % 2 == 0) {
        if (rightInvader == true) {
            InvaderShip->Position.x += 5;
            if (InvaderShip->Position.x >= (SCREEN_WIDTH - 70)) {
                rightInvader = false;
                InvaderShip->Position.x -= 5;
                InvaderShip->Position.y += 70;
            }
        } else {
            InvaderShip->Position.x -= 5;
            if (InvaderShip->Position.x <= 10) {
                rightInvader = true;
                InvaderShip->Position.x += 5;
                InvaderShip->Position.y += 70;
            }
